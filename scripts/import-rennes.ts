...
const recordSchema = z.object({
  ...
  geo_shape: z.object({ geometry: z.object({ type: z.literal("Point"), coordinates: z.tuple([z.number(), z.number()]) }) }),
  localisation: z.string().nullable(), abattu: z.union([z.literal(0), z.literal(1), z.null()]),
  nom_commun: z.string().nullable(), genre: z.string().nullable(), espece: z.string().nullable(), variete: z.string().nullable(),
  hauteur: z.number().nullable(), circonference: z.number().nullable(),
  date_plantation: z.string().datetime({ offset: true }).nullable(), date_maj: z.string().datetime({ offset: true }).nullable(),
  id_gestion: z.string().nullable(), complement: z.string().nullable(), type_taille: z.string().nullable(), fonction: z.string().nullable(),
}).passthrough();

function meaningfulText(value: string | null) {
  const text = value?.trim();
  return !text || /^(non renseign[eé]|inconnu|sans objet)$/i.test(text) ? null : text;
}
...
export function normalizeRecord(input: unknown) {
  ...
  return { feature: treeFeatureSchema.parse({
    type: "Feature", id: `rennes-arbre-${record.id}`,
    ...
    properties: {
      ...
      description: meaningfulText(record.complement),
      date_plantation: record.date_plantation, date_maj: record.date_maj,
      localisation: record.localisation, type_taille: meaningfulText(record.type_taille),
      fonction: meaningfulText(record.fonction), source_properties: raw,
    },
  }) };
}

const [west, south, east, north] = PARK_BOUNDS;
export const WHERE = `within(geo_shape, geom'POLYGON((${west} ${south},${east} ${south},${east} ${north},${west} ${north},${west} ${south}))')`;

async function getJson(url: string | URL) {
  const response = await fetch(url, { signal: AbortSignal.timeout(30_000) });
  if (!response.ok) throw new Error(`API Rennes : HTTP ${response.status}`);
  return response.json() as Promise<unknown>;
}

export async function importRennes() {
  const metadata = z.object({ metas: z.object({ default: z.object({
    license: z.string().min(1), license_url: z.string().url(), data_processed: z.string().datetime({ offset: true }).nullable(),
  }) }) }).parse(await getJson(API_URL)).metas.default;
  if (metadata.license !== "Licence ODbL 1.0") throw new Error("La licence de la source a changé : vérifier avant de publier.");
  const query = new URL(`${API_URL}/records`);
  ...
  do {
    ...
    if (total > 5000) throw new Error("Volume inattendu pour cette emprise : import interrompu.");
    if (records.length < total && !page.results.length) throw new Error("Pagination incomplète.");
    records.push(...page.results);
  } while (records.length < total);
  if (records.length !== total) throw new Error("Nombre de résultats incohérent.");
  const normalized = records.map(normalizeRecord);
  const features = normalized.flatMap((item) => item.feature ? [item.feature] : []);
  const collection = treeCollectionSchema.parse({
    type: "FeatureCollection", bbox: PARK_BOUNDS,
    metadata: {
  ...
  const target = new URL("arbres-rennes.geojson", folder);
  const temporary = new URL(`arbres-rennes.${process.pid}.tmp`, folder);
  ...
  console.log(`${features.length} arbres importés sur ${records.length} points dans l’emprise.`);
  console.log(`GeoJSON validé : ${fileURLToPath(target)}`);
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  importRennes().catch((error: unknown) => {
    console.error("Import interrompu. Le précédent fichier reste intact.", error);
    process.exitCode = 1;
  });
}
